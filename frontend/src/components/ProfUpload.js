...
export default function ProfUpload() {
    ...
    async function convertToText(file) {
        ...
        setSemantics(prevSemantics => {
            const newSemantics = [...prevSemantics, ...results];
            return newSemantics;
        });
    }
    async function handleUpload() {
        ...
        formData.append('panswertext', semantics.join(" "));
        formData.append('username', currentUser.username);
        for (const pair of formData.entries()) {
            console.log(`${pair[0]}, ${pair[1]}`);
        }
        const request = await fetch("http://localhost/profupload", {
            method: "POST",
            mode: "cors",
            body: formData
        })
                                ...
                                {file.map((file, index) => {
                                    ...
                                    return <div className={`each-file  rounded p-2 m-2 col-md-3 col-6 `} key={index}>
                                        ...
                                        <div className="container semantic-res p-2 m-1 rounded" style={{ fontSize: "small", backgroundColor: "#FF66006b", color: "black", fontWeight: "400" }}>
                                            <span style={{ fontWeight: "500" }}>Text Extracted: </span> <span > {semantics[index] != null ? semantics[index] : "fetching..."}</span>
                                        </div>
                                    </div>
